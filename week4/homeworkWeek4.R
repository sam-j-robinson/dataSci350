...
if(interactive()){
  ...
  loginfo("Title: Chicago Data LM")
  loginfo("Script By: Sam Robinson")
  
  # Setup working directory and import data
  ...
  #Convert the data into a table to feed into our lm model.
  ...
  #Create a data table for the diffed data
  ...
  #Plotting the model and getting the summary
  #ggplot(diff_data, aes(x=hospitalizations_diff, y=admit_rate_diff)) + 
  #  geom_point() + 
  #  geom_smooth(method='lm')  
  
  diff_hospital_data_lm <- lm(diff_data$hospitalizations_diff ~ diff_data$admit_rate_diff)
  
  log_analysis(crude_hospital_data_lm, diff_hospital_data_lm)
}